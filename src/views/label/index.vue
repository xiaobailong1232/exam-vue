<template>
  <div class="app-container">
    ...
    <!-- 弹出层：创建标签 start -->
    <el-dialog title="创建标签" :visible.sync="form.show" :width="'400px'">
  ...
  export default {
    ...
    methods: {
      ...
      initFetch() {
        ...
        this.loading = true;
        getLabelListFromApi(this.search).then(response => {
      ...
      addItem() {
        ...
        addLabelItemToApi(this.form.data).then(response => {
      ...
      deleteItem(row) {
        ...
        .then(() => deleteLabelItemToApi(row.id))
        .then(response => {
