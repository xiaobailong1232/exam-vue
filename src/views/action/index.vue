  ...
  <div class="app-container">
    ...
    <el-dialog title="创建功能" :visible.sync="form.show" :width="'400px'">
      <el-form :model="form.data" :label-width="'100px'">
        ...
        <el-form-item label="action">
          <el-input v-model="form.data.action" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog :title="edit.status ? '编辑' : '详情'" :visible.sync="edit.show">
      <el-form :model="edit.data" :label-width="'120px'">
        ...
        <el-form-item label="action">
          <el-input v-model="edit.data.action" auto-complete="off"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  import {
    getActionListFromApi,
    addActionItemToApi,
    updateActionItemToApi,
    deleteActionItemToApi,
    restoreActionItemToApi,
  } from '@/api/action';
  ...
  export default {
    ...
    methods: {
      ...
      initFetch() {
        ...
        this.loading = true;
        getActionListFromApi(this.search).then(response => {
      ...
      addItem() {
        ...
        addActionItemToApi(this.form.data).then(response => {
      ...
      showDetailForm(row) {
        ...
        this.edit.data.action = row.action;
      },
      // 编辑信息
      showEditForm(row) {
      ...
      deleteItem(row) {
        ...
        .then(() => deleteActionItemToApi(row.id))
        .then(response => {
